# GitHub Top Targets

Этот документ фиксирует первую сильную волну GitHub-целей для `Дом рекламы / Advhouse` вокруг мирового рекламного, креативного, медийного и digital-контура.

Цель не в том, чтобы хаотично подписаться на сотни случайных аккаунтов, а в том, чтобы собрать рабочий внешний круг влияния:

- платформы и компании из контура `Cannes Lions`;
- компании, связанные с креативом, медиа, дизайном и digital;
- технические и продуктовые репозитории, которые реально формируют современную среду коммуникаций.

Отдельная рабочая карта по трем направлениям лежит здесь:

- [github-direction-targets.md](./github-direction-targets.md)

Важно: у большинства крупных рекламных премий GitHub не является их основной публичной площадкой. Поэтому основной вес здесь переносится на компании, платформы, партнеров, спикеров и технологические организации вокруг этих премий.

## Как читать этот список

- `Follow account`: следить за организацией или компанией как за публичным GitHub-узлом.
- `Star repo`: сохранить конкретный сильный репозиторий как ориентир, инструмент или сигнал внимания.
- `Основной круг`: без него внешний контур будет неполным.
- `Каннский круг`: партнеры, жюри, спикеры и компании из публичного контура `Cannes Lions`.
- `Среда премий`: люди и компании из `D&AD`, `Clio`, `The One Show`, `Effie`, `ADC`, `LIA`, `Epica`, `Eurobest`.

## Три круга

### 1. Основной круг

Это обязательный слой. Он собирает главные компании и платформы, без которых нельзя серьезно наблюдать за современной средой рекламы, дизайна, digital и креативных коммуникаций.

`За кем следить`

1. [adobe](https://github.com/adobe)
2. [AdobeDocs](https://github.com/AdobeDocs)
3. [aws](https://github.com/aws)
4. [Canva](https://github.com/Canva)
5. [linkedin](https://github.com/linkedin)
6. [microsoft](https://github.com/microsoft)
7. [google](https://github.com/google)
8. [figma](https://github.com/figma)
9. [github](https://github.com/github)
10. [openai](https://github.com/openai)
11. [vercel](https://github.com/vercel)
12. [cloudflare](https://github.com/cloudflare)

`Какие репозитории отмечать`

1. [adobe/react-spectrum](https://github.com/adobe/react-spectrum)
2. [adobe/leonardo](https://github.com/adobe/leonardo)
3. [AdobeDocs/uxp-photoshop](https://github.com/AdobeDocs/uxp-photoshop)
4. [google/material-design-icons](https://github.com/google/material-design-icons)
5. [figma/plugin-samples](https://github.com/figma/plugin-samples)
6. [figma/code-connect](https://github.com/figma/code-connect)
7. [microsoft/vscode](https://github.com/microsoft/vscode)
8. [microsoft/TypeScript](https://github.com/microsoft/TypeScript)
9. [aws/aws-cdk](https://github.com/aws/aws-cdk)
10. [aws/aws-cli](https://github.com/aws/aws-cli)
11. [google/styleguide](https://github.com/google/styleguide)
12. [vercel/next.js](https://github.com/vercel/next.js)
13. [cloudflare/pingora](https://github.com/cloudflare/pingora)
14. [openai/openai-cookbook](https://github.com/openai/openai-cookbook)
15. [github/github-mcp-server](https://github.com/github/github-mcp-server)

### 2. Каннский круг

Это слой `Cannes Lions`: партнеры, участники публичной программы, технологические платформы, а также компании из жюри и креативно-маркетингового контура, у которых есть реальный GitHub-след.

Важно: не у всех партнеров и спикеров `Cannes Lions` есть живой GitHub, поэтому в слой включены только те, у кого GitHub действительно является рабочим публичным узлом.

`За кем следить`

1. [reddit](https://github.com/reddit)
2. [Infosys](https://github.com/Infosys)
3. [mediamonks](https://github.com/mediamonks)
4. [Accenture](https://github.com/Accenture)
5. [salesforce](https://github.com/salesforce)
6. [Netflix](https://github.com/Netflix)

`Какие репозитории отмечать`

1. [reddit/redditsans](https://github.com/reddit/redditsans)
2. [reddit/achilles-sdk](https://github.com/reddit/achilles-sdk)
3. [linkedin/Liger-Kernel](https://github.com/linkedin/Liger-Kernel)
4. [linkedin/school-of-sre](https://github.com/linkedin/school-of-sre)
5. [Netflix/chaosmonkey](https://github.com/Netflix/chaosmonkey)
6. [Netflix/eureka](https://github.com/Netflix/eureka)
7. [Infosys/Infosys-Agentic-Foundry](https://github.com/Infosys/Infosys-Agentic-Foundry)
8. [Infosys/Infosys-Responsible-AI-Toolkit](https://github.com/Infosys/Infosys-Responsible-AI-Toolkit)
9. [Accenture/AmpliGraph](https://github.com/Accenture/AmpliGraph)
10. [salesforce/LAVIS](https://github.com/salesforce/LAVIS)
11. [mediamonks/muban](https://github.com/mediamonks/muban)
12. [mediamonks/fast-image-sequence](https://github.com/mediamonks/fast-image-sequence)

### 3. Среда премий

Это круг наблюдения за более широкой средой `D&AD`, `Clio`, `The One Show`, `Effie`, `ADC`, `LIA`, `Epica`, `Eurobest`: люди, компании, платформы и продуктовые команды, за которыми действительно стоит следить.

Здесь GitHub чаще связан не с самими award-брендами, а с их средой:

- агентствами;
- design- и product-командами;
- martech и media-tech;
- commerce и digital-инфраструктурой.

`За кем следить`

1. [Shopify](https://github.com/Shopify)
2. [IBM](https://github.com/IBM)
3. [akqa](https://github.com/akqa)

`Какие репозитории отмечать`

1. [IBM/plex](https://github.com/IBM/plex)
2. [Shopify/react-native-skia](https://github.com/Shopify/react-native-skia)
3. [Shopify/liquid](https://github.com/Shopify/liquid)
4. [google/zx](https://github.com/google/zx)
5. [microsoft/markitdown](https://github.com/microsoft/markitdown)
6. [microsoft/generative-ai-for-beginners](https://github.com/microsoft/generative-ai-for-beginners)
7. [mediamonks/frontend-coding-standards](https://github.com/mediamonks/frontend-coding-standards)
8. [aws/serverless-application-model](https://github.com/aws/serverless-application-model)
9. [linkedin/css-blocks](https://github.com/linkedin/css-blocks)

## Направления

- `Реклама`
- `Стратегия`
- `Пиар`
- `Медиа`
- `Диджитал`
- `Дизайн`
- `Креативные коммуникации`

## Почему именно эти цели

### Реклама

- Adobe
- Canva
- Monks
- LinkedIn
- Accenture

### Стратегия

- Infosys
- Salesforce
- Microsoft
- Accenture
- Google

### Пиар

- Reddit
- LinkedIn
- Monks

### Медиа

- Reddit
- LinkedIn
- Netflix
- Amazon

### Диджитал

- Microsoft
- Google
- AWS
- Vercel
- Cloudflare
- Shopify
- Figma

### Дизайн

- Adobe
- Figma
- Canva
- IBM Plex
- Google Material

### Креативные коммуникации

- Adobe
- Canva
- LinkedIn
- Reddit
- Monks

## Премиальный контекст

Эти премии остаются обязательным контекстом наблюдения:

- `Cannes Lions`
- `D&AD Awards`
- `Clio Awards`
- `The One Show`
- `Effie Awards`
- `ADC Annual Awards`
- `London International Awards`
- `Epica Awards`
- `Eurobest`

Но основное присутствие на GitHub строится не вокруг самих премий, а вокруг:

- партнеров фестивалей;
- спикеров и жюри;
- технологических платформ;
- агентств и продуктовых компаний;
- дизайн- и digital-систем.
